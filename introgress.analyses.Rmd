...
#dev.off()
#make triangle plot
plot(x=hi.index.sim$h, y=het, pch=21, cex=2, col=rgb(0,0,0,alpha=.3),
     bg=c("green","green",rep("plum", times=4),rep("green",times=20),rep("red", times=4), alpha=.3),
...
#plot
pdf(file="~/Desktop/parrots/introgress.fig.pdf", width=12,height=8)
mk.image(introgress.data=count.matrix, loci.data=locus.info.filtered,
         marker.order=NULL,hi.index=hi.index.sim, ylab.image="Samples",
         xlab.h="Red-crowned ancestry", pdf=F,
         col.image=c("red","grey22","plum"))
dev.off()

setEPS()
postscript("~/Desktop/parrots/allelic.mismatch.plot.eps")
mk.image(introgress.data=count.matrix, loci.data=locus.info.filtered,
         marker.order=NULL,hi.index=hi.index.sim, ylab.image="Samples",
         xlab.h="Red-crowned ancestry", pdf=F,
         col.image=c("red","grey22","plum"))
...
```
Thisplot shows 214 fixed SNPs with 50% missing data cutoff in both parental pops. 
...
dev.off()
#make triangle plot
pdf(file="~/Desktop/parrots/triangle.plot.pdf", width=5,height=5)
plot(x=hi.index.sim$h, y=het, pch=21, cex=1.1, col=rgb(0,0,0,alpha=1),
     bg=c("green","green",rep("plum", times=4),rep("green",times=20),rep("red", times=4)),
     xlab="Hybrid Index", ylab="Interspecific heterozygosity",
     ylim=c(0,1))
segments(x0 =0, y0 =0, x1 =.5, y1 =1)
segments(x0 =1, y0 =0, x1 =.5, y1 =1)
...
setEPS()
postscript("~/Desktop/parrots/triangle.plot.eps")
plot(x=hi.index.sim$h, y=het, pch=21, cex=1.1, col=rgb(0,0,0,alpha=1),
     bg=c("green","green",rep("plum", times=4),rep("green",times=20),rep("red", times=4)),
     xlab="Hybrid Index", ylab="Interspecific heterozygosity",
     ylim=c(0,1))
...
This reduced dataset makes it seem like almost all of the introgressed alleles are exlusively found as het sites, 
which didn't really seem to be the case when we allowed more missing data above.
...
Because of our skewed sample size in LA between RC and LC, we have more statistical power to detect 
introgression from LC -> RC than vice versa. For this reason we highlight significant LC->RC outliers.

```{r}
#plot all the clines overlayed, and color the LC->RC significant outliers in red
cline.data<-cline.object

#pdf(file="~/Desktop/parrots/genomic.cline.outliers.pdf", width=4,height=4)
## homozygote
plot(0:1,0:1,type="n",xlab="Hybrid index",ylab="Lilac-crowned allele frequency")
